...
**Answer**
It shows 131395 Observations and 24 variables
### Task 3: Glimpse data
...
**Answer**
last_name is a chr column, last name being donors last name and amount being how much that person donated. 
...
```{r}
# the name of our data set, |> is a pipe operator 
# say the name of our data set, and then tell R we're going to do something with that data set
maryland_winred_contributions |>
  # group together all the identical values in teh data column, and then do Something else with the data
  group_by(date) |>
  #let's do soemthing with those groupings: 
  summarise(
    # create a new colmn called count_contribs that equals the number of rows that have the same date value
    count_contribs = n()
  )
```
...
**Answer**
It summarizes the how many donations were given in the data set on each particular date. 
...
maryland_winred_contributions |>
  ...
  # arranging the data in decending order (largest to smallest) by teh values in count_contribs
...
**Answer**
Instead of the data being organized by chronological date, it is organized from most contributes given at the top to least contributions at the bottom of the list.
...
**Answer**
Now it shows the amount of money that was donated for the contributes. 
...
**Answer**
The data shows by committe how many contributions were made. 
...
**Answer**
Now it is grouped by what organization money was given to, along with what the mean amount of money was donated for those organizations.
### Task 11
...
**Answer**
Now the data shows that it is grouped by city in alphabetical order. 
